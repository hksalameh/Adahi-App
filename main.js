...
// تهيئة Firebase (تتم بالفعل داخل auth.js و firestoreService.js إذا كانا يستوردان config)
// const app = initializeApp(firebaseConfig); // لا حاجة لإعادة التهيئة هنا

let unsubscribeAdminSacrifices = null;
let unsubscribeUserSacrifices = null;
let currentEditingDocId = null; // لتتبع المستند الذي يتم تعديله

// --- مستمعو أحداث واجهة المستخدم ---
...
const loginFormEl = document.getElementById('loginForm'); // تعريف جديد هنا
if (loginFormEl) {
    loginFormEl.addEventListener('submit', async (e) => {
        ...
        try {
            const result = await loginUser(usernameOrEmail, password, rememberMe);
            if (!result.success) { // إذا كان loginUser يعيد كائنًا مع خاصية success
                 ui.authStatusEl.textContent = result.error || 'فشل تسجيل الدخول.';
                 ui.authStatusEl.className = 'error';
            }
            // onAuthStateChanged سيتولى تحديث الواجهة عند النجاح
        } catch (error) {
...
onAuthStateChanged(auth, async (user) => {
    // إلغاء الاشتراكات القديمة أولاً
    ...
    if (user) {
        ...
        if (user.uid === ADMIN_UID) {
            fetchAndRenderSacrificesForAdmin(null); // جلب كل البيانات مبدئيًا
        } else {
...
if (ui.adahiForm) {
    ui.adahiForm.addEventListener('submit', async (event) => {
        ...
        if (currentEditingDocId) { // استخدام currentEditingDocId الذي يتم تعيينه بواسطة populateAdahiFormForEdit
            ...
            try {
                ...
                ui.resetAdahiFormToEntryMode({ current: currentEditingDocId }); // تمرير الحالة لإعادة تعيينها
                currentEditingDocId = null; // إعادة التعيين هنا أيضًا
...
function renderSacrificesForAdminUI(docs) {
    ...
    docs.forEach((docSnapshot) => {
        ...
        editButton.onclick = () => ui.populateAdahiFormForEdit(docSnapshot.id, data, { current: currentEditingDocId, set: (id) => currentEditingDocId = id });
...
function fetchAndRenderSacrificesForAdmin(statusFilter = null) {
    ...
    if (unsubscribeAdminSacrifices) unsubscribeAdminSacrifices(); // إلغاء الاشتراك السابق
...
function fetchAndRenderSacrificesForUserUI(userId) {
    ...
    if (unsubscribeUserSacrifices) unsubscribeUserSacrifices(); // إلغاء الاشتراك السابق
...
const exportAllToCsvButtonEl = document.getElementById('exportAllToCsvButton'); // تعريف جديد
if (exportAllToCsvButtonEl) {
    exportAllToCsvButtonEl.addEventListener('click', async () => {
        ...
        try {
            // لا نستخدم onSnapshot هنا، بل getDocs مرة واحدة للتصدير
            const querySnapshot = await getDocs(query(collection(db, "sacrifices"), orderBy("createdAt", "desc")));
...
const exportAllUsersSeparateCsvButtonEl = document.getElementById('exportAllUsersSeparateCsvButton'); // تعريف جديد
if (exportAllUsersSeparateCsvButtonEl) {
    exportAllUsersSeparateCsvButtonEl.addEventListener('click', async () => {
        ...
        try {
            // ملاحظة: قد تحتاج إلى فهرس مركب (userId, createdAt) في Firestore
            const querySnapshot = await fsService.getAllSacrificesForExport(); // استخدام الدالة من firestoreService
            ...
            if (error.code === 'failed-precondition' && error.message.includes('index')) {
                 ui.statusMessageEl.textContent = "خطأ: يتطلب هذا التصدير فهرسًا مركبًا. يرجى التحقق من وحدة التحكم في Firebase لإنشاء الفهرس: userId (تصاعدي), createdAt (تنازلي).";
...
// لتهيئة عرض النموذج بشكل صحيح عند تحميل الصفحة أول مرة (قبل أي تفاعل)
document.addEventListener('DOMContentLoaded', () => {
    ui.resetAdahiFormToEntryMode();
    // تأكد من أن الحقول الشرطية مخفية بشكل صحيح
