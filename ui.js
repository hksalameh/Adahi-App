...
export const adminLoadingMessage = document.getElementById('loadingMessage');
...
export function showAuthenticatedView(isAdmin) {
    authStatusEl.textContent = `مرحباً بك`;
...
export function resetAdahiFormToEntryMode(editingDocIdStateSetter) {
    adahiForm.reset();
    if (editingDocIdStateSetter) editingDocIdStateSetter(null);
...
export function populateAdahiFormForEdit(docId, data, editingDocIdStateSetter) {
    if (editingDocIdStateSetter) editingDocIdStateSetter(docId);
    ...
    if (!cancelButton) {
        ...
        cancelButton.onclick = () => resetAdahiFormToEntryMode(editingDocIdStateSetter);
